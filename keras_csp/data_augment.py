...
def expend(image, gts, igs, pave_size, limit=8):
    img_height, img_width = image.shape[0:2]
    exp_h, exp_w = pave_size
    if img_height < exp_h:
        expend_image = np.ones((exp_h, img_width, 3), dtype=image.dtype) * np.mean(image, dtype=int)
        expend_image[0:img_height, 0:img_width, :] = image
    elif img_width < exp_w:
        expend_image = np.ones((img_width, exp_w, 3), dtype=image.dtype) * np.mean(image, dtype=int)
        expend_image[0:img_height, 0:img_width, :] = image
    else:
        expend_image = image
    cv2.imwrite('./output/output.jpg', expend_image)
    return expend_image, gts, igs
...
def augment(img_data, c):
    ...
    if img.shape[0] < c.size_train[0] and img.shape[1] >= c.size_train[1]:
        img, gts, igs = expend(img, gts, igs, c.size_train, limit=16)
    elif img.shape[0] >= c.size_train[0] and img.shape[1] < c.size_train[1]:
        img, gts, igs = expend(img, gts, igs, c.size_train, limit=16)
